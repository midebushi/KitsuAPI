...
function RouteComponent() {
  ...
  const allManga = data?.pages.flatMap((page) => page) || []
    ...
    <div className='mx-auto flex-4 w-full'>
      <div className='px-5'>
        <h2 className='text-theme-text-primary font-bold text-3xl'>
          {debouncedSearch ? `Search results for "${debouncedSearch}"` : `Search in manga`}
        </h2>
      ...
      <div className=''>
        <MediaList
          ...
          error={error}
          renderItem={(item) => (
            <Link key={item.id} to='/manga/$mangaId' params={{ mangaId: item.id }}>
              <Card item={item} />
            </Link>
          )}
